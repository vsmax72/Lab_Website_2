...
function loadStatsPage()
{
	...
	for(row_counter = 2; row_counter<table.rows.length; row_counter++)
	{
			...
			{
				winner_name = table.rows[row_counter].cells["1"].innerHTML;
				loss ++;
			}
			else
			{
				winner_name = "CU Boulder";
				win ++;
			}
			table.rows[row_counter].cells["4"].innerHTML = winner_name;

	}
	document.getElementById("wins").innerHTML = parseInt(win);
	document.getElementById("losses").innerHTML = parseInt(loss);
...
function loadPlayersPage()
{
	...
	for(var i=0; i<players.length; i++)
	{
		...
		elem.href = "#";
		elem.setAttribute("onclick", "switchPlayers(" + i + ")");
...
function switchPlayers(playerNum)
{
	...
	document.getElementById("avg_rec_yards").innerHTML = parseInt((players[playerNum].receiving_yards)/(player[playerNum].games_played));
